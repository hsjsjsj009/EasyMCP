...
use rmcp::handler::server::tool::{
    CallToolHandler, FromToolCallContextPart, IntoCallToolResult, ToolCallContext,
};
...
impl<T0, S, R> CallToolHandler<S, PhantomData<dyn Fn(T0) -> R>> for DynamicMCPClosure<T0, R>
where
