...
use rmcp::model::{CallToolResult, Content, ErrorCode, JsonObject, ServerCapabilities, ServerInfo, Tool, ToolAnnotations};
...
impl DynamicMCP {
    ...
    pub fn new_from_file(file_path: &str) -> Self {
        let data: DynamicMCPConfig = match serde_yaml::from_str(file_path) {
            Ok(data) => data,
            Err(err) => panic!(
        ...
        Self::new(data.tools, data.instruction)
    }

    pub fn new(tool_data: Vec<ToolData>, instruction: String) -> Self {
        Self {
    ...
    fn general_http_method_template(
        ...
        template.add_template(Self::URL_TEMPLATE_NAME, &url).expect("Error registering url template");

        let body_exist = if let Some(ref body_str) = body_template {
            template.add_template(Self::BODY_TEMPLATE_NAME, body_str).expect("Error registering body template");
            true
        } else {
            false
        };
        ...
        let header_template_names: HashMap<String, String> = header_template.keys().map(|name| {
            (name.clone(), Self::http_header_template_name(name.clone()))
        }).collect();
        ...
        for (header_name, template_name) in header_template_names.iter() {
            if let Some(header_value) = header_template.get(header_name) {
                if let Err(err) = template.add_template(template_name, header_value) {
                    panic!("Error while registering header template '{}': {}", header_name, err);
                }
            }
        }
        ...
        for entry in tool_data.iter() {
            match entry.tool_type {
                ToolType::HTTP => {
                    ...
                    let closure = Self::general_http_method_template(method, url, body_template, headers);
...
impl ServerHandler for DynamicMCP {
    fn get_info(&self) -> ServerInfo {
        ServerInfo {
            instructions: Some(self.instruction.clone()),
